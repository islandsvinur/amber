...
\begin{enumerate}
  ...
  \item The launch velocity is dependent on how ``big'' the story is (like from
        an important author or it has many references).
  \item Stories can get velocity boosts by getting replies/comments or
        references (``trackbacks'') in order to keep them around.
  \item FIXME: There is a number of dark matter bodies for each subject which
        attract stories in some amount, depending on their subject.
\end{enumerate}

FIXME: Think about difference between static and dynamic view\ldots
